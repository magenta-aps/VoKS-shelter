<div class="side-block waiting-list" ng-controller="WaitingLineController">
    <div class="side-block__title">
        <div class="side-block__text">{{ Lang::get('waiting-line.label') }} <span>[<% (clients | filter: {position: {inQueue: true} }).length %>]</span></div>
        <div class="side-block__select">
            <ui-select ng-model="dropdown.order">
                <ui-select-match placeholder="{{ Lang::get('waiting-line.select_order') }}"><% $select.selected.title %></ui-select-match>
                <ui-select-choices repeat="option in config['order-options'] | filter: $select.search">
    ...
    <div class="side-block__container">
        ...
        <div class="waiting-list__lines" perfect-scrollbar>
            ...
            '-chat-open': client.state.chatOpen
                ...
                <a class="button icons -active" ng-show="client.state.calling" ng-class="
                    {
